...
For private registries, the inspect command uses the credentials from '~/.docker/config.json'.`,
	Example: `  # Inspect an OCI artifact
  kustomizer inspect oci://docker.io/user/repo:latest

  # List only the container images references
  kustomizer inspect oci://docker.io/user/repo:v1.0 --container-images
`,
	RunE: runInspectCmd,
}
...
var inspectArgs inspectFlags

func init() {
	inspectCmd.Flags().BoolVar(&inspectArgs.containerImages, "container-images", false,
...
func runInspectCmd(cmd *cobra.Command, args []string) error {
	...
	if inspectArgs.containerImages {
